function New-Livery {
    if (-not (Test-Path $livloc)) {
        ...
        New-Item -Path $livloc\SimObjects\Airplanes\Asobo_B787_10_$airfold\MODEL.$airfold -ItemType Directory
        New-Item -Path $livloc\SimObjects\Airplanes\Asobo_B787_10_$airfold\MODEL.AI_$airfold -ItemType Directory
        New-Item -Path $livloc\SimObjects\Airplanes\Asobo_B787_10_$airfold\TEXTURE.$airfold -ItemType Directory
    }
}
function Move-Files {
    ...
    foreach ($item in $moveddsfile) {
        Copy-Item $PSScriptRoot\texture\json\$item.json -destination "$livloc\SimObjects\Airplanes\Asobo_B787_10_$airfold\TEXTURE.$airfold"
    }
    ...
    Copy-Item $PSScriptRoot\Files\aircraft.cfg $livloc\SimObjects\Airplanes\Asobo_B787_10_$airfold\aircraft.cfg
    Copy-Item $PSScriptRoot\Files\model.cfg $livloc\SimObjects\Airplanes\Asobo_B787_10_$airfold\MODEL.$airfold\model.cfg
    Copy-Item $PSScriptRoot\Files\model.cfg $livloc\SimObjects\Airplanes\Asobo_B787_10_$airfold\MODEL.AI_$airfold\model.cfg
    Copy-Item $PSScriptRoot\Files\texture.cfg $livloc\SimObjects\Airplanes\Asobo_B787_10_$airfold\TEXTURE.$airfold\texture.cfg
    Get-ChildItem -path $PSScriptRoot\texture -filter *.dds | Copy-Item -destination "$livloc\SimObjects\Airplanes\Asobo_B787_10_$airfold\TEXTURE.$airfold"
}
...
function Update-AircraftConfig {
    ...
    $file = Get-content -path "$livloc\SimObjects\Airplanes\Asobo_B787_10_$airfold\aircraft.cfg"
    $newfile = $file -replace 'AIRFOLD" ', ("$airfold" + '" ')
    $newfile | Set-content -path "$livloc\SimObjects\Airplanes\Asobo_B787_10_$airfold\aircraft.cfg"
    $file = Get-content -path "$livloc\SimObjects\Airplanes\Asobo_B787_10_$airfold\aircraft.cfg"
    $newfile = $file -replace 'AIRLINE" ', ("$airline" + '" ')
    $newfile | Set-content -path "$livloc\SimObjects\Airplanes\Asobo_B787_10_$airfold\aircraft.cfg"
    $file = Get-content -path "$livloc\SimObjects\Airplanes\Asobo_B787_10_$airfold\aircraft.cfg"
...
function Open-Blender {
    ...
    Start-Process -FilePath $blenderloc.FullName -ArgumentList $PSScriptRoot\787.blend
    Write-Host "Waiting until you are complete with editing the files"
    Pause
}

function Expand-BlenderFile {
    Write-Host "Expanding 787.zip"
    Expand-Archive "$PSScriptRoot\787.zip" -DestinationPath $PSScriptRoot -Force
...
function Get-PackageLoc {
    ...
    $finally = $maybe.Split(" ")
    $script:livloc = $finally[1].Replace('"',"") + "\Community" + "\$name-b787-livery-$airfold"
}

# Form boxes working on making it one box.
[void][System.Reflection.Assembly]::LoadWithPartialName('Microsoft.VisualBasic')
$script:name = [Microsoft.VisualBasic.Interaction]::InputBox("Enter your name", "Persona").toupper()
